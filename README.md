...
<p align="center">
  ...
  <p align="center">
    <i>
    A simple E-commerce web aplication created with React during a web developer bootcamp by 4 awesome students that may already be full flegded developpers.
    </i>
    <br />
    <br />
...
## About the code
The following are the major framework and technologies that we used to build our project:
- React
- React Router
- Styled Components
- ...
...
In our project we mostly used the `useState`, `useEffect`, `useRef` and `useContext` hooks.

[Go back to the TOC](#table-of-contents)

### Context
In our project we decided to use *React Context* because it allows us to share information to any component by storing it in a central place and allowing access to any component that requests it (usually you are only able to pass data from parent to child via props).

With this approach, it’s quite easy to provide to any and all components in the React component tree, regardless of how deeply nested they may be.

To create a new Context, we use the React `createContext()`  function.

To access the context value, we use `useContext()` React hook.
...
Out of the different solutions at our disposal, we chose the Stripe API because it’s well documented and its quite ease to integrate into projects.

We are using it in *Testing mode* for this webapp but we can switch it to receive real payments quite easily.
...
### `theme.js` and `textData.js`
In order to make our project scalable and maintainable, we opted to keep everything related to the style and text inside two single files.

All texts and related information are stored inside a file called `textData.js`. This file contains the strings that are injected into the code so that they are shown to the final user when the page is loaded.
...
- Time-boxed: 2 months of calendar time but actually just ~48 effective hours.
...
We are pretending to be a Scrum team working under a fictional company called “Norris Inc.” that sells workwear through with a “direct-to-consumer” business model through an online E-commerce platform. The clothes are made by a third party to our specifications.
...
- Also has a desktop / laptop PC but it's old one with a small screen that is used primarly for Facebook and emails.
...
#### Scrum?
When it was time to begin working, we realized that we were not completely comfortable with Scrum so after talking about it we decided run what we called a “mock sprint” with one team member acting as a "Scrum Master" and another one as a "Product Owner".
